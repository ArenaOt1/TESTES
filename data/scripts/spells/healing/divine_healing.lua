...
function onGetFormulaValues(_player, level, magicLevel) -- already compared to the official tibia | compared date: 05/07/19(m/d/y)
	local min = (level * 0.2 + magicLevel * 10.22) + 44 -- GLOBAL local min = (level * 0.2 + magicLevel * 7.22) + 44
	local max = (level * 0.2 + magicLevel * 15.79) + 79 -- local max = (level * 0.2 + magicLevel * 12.79) + 79
	return min, max
end
