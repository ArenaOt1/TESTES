...
function onGetFormulaValues(player, level, magicLevel) -- already compared to the official tibia | compared date: 05/07/19(m/d/y)
	local min = (level * 0.2 + magicLevel * 18) + 75 -- GLOBAL local min = (level * 0.2 + magicLevel * 12) + 75
	local max = (level * 0.2 + magicLevel * 30) + 125 -- GLOBAL local max = (level * 0.2 + magicLevel * 20) + 125
	return min, max
end
