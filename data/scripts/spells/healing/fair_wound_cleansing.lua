...
function onGetFormulaValues(_player, level, magicLevel) -- already compared to the official tibia | compared date: 05/07/19(m/d/y)
	local min = (level * 0.2 + magicLevel * 6 + 25) * 2 -- local min = (level * 0.2 + magicLevel * 4 + 25) * 2
	local max = (level * 0.2 + magicLevel * 9.95 + 51) * 2 -- local max = (level * 0.2 + magicLevel * 7.95 + 51) * 2
	return min, max
end
