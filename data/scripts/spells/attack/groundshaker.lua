...
function onGetFormulaValues(player, skill, attack, factor)
	...
	local min = (level / 5) + (skill + attack) * 1.0 -- GLOBAL local min = (level / 5) + (skill + attack) * 0.5
	local max = (level / 5) + (skill + attack) * 2.3 -- GLOBAL local max = (level / 5) + (skill + attack) * 1.1
