...
function onGetFormulaValues(player, skill, attack, factor)
	...
	local min = (level / 5) + (skill + 2 * attack) * 2.6
	local max = (level / 5) + (skill + 2 * attack) * 7
...
function onGetFormulaValues(player, skill, attack, factor)
	local level = player:getLevel()
	local min = (level / 5) + (skill + 3 * attack) * 3.2
	local max = (level / 5) + (skill + 3 * attack) * 8 
	return -min * 1.1, -max * 1.1 
end
else 
function onGetFormulaValues(player, skill, attack, factor)
	local level = player:getLevel()
	local min = (level / 5) + (skill + 2 * attack) * 2.2
	local max = (level / 5) + (skill + 2 * attack) * 6
